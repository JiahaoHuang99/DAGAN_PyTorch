...
# Original U_Net
class Original_U_Net(nn.Module):
    def __init__(self):
        super(Original_U_Net, self).__init__()
        ...
        # network
        self.conv1 = nn.Sequential(
            nn.Conv2d(in_channels=1, out_channels=self.gf_dim, kernel_size=self.kernel_size, stride=1,
                      padding=self.padding),
            nn.LeakyReLU(negative_slope=0.2)
        )
